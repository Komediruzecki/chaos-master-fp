...
export function generateTransformId(identifier: string = ''): TransformId {
  // eslint-disable-next-line no-restricted-globals -- cross-runtime (Deno + browser)
  return `_${identifier}__${crypto
    .randomUUID()
...
export function generateVariationId(): VariationId {
  // eslint-disable-next-line no-restricted-globals -- cross-runtime (Deno + browser)
  return crypto.randomUUID().replaceAll('-', '_') as VariationId
}
