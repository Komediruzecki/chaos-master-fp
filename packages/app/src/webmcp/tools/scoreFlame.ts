...
export function calculateFlameStats(flame: FlameDescriptor) {
  ...
  // 3. Symmetry Score
  // Detect auxiliary symmetry transforms (_sym__ prefix) generated by sidebar symmetry and arena
  const symTransforms = Object.entries(flame.transforms ?? {}).filter(
    ([k, t]) => k.startsWith('_sym__') && (t.visible ?? true),
  ).length

  // Structural symmetry: C2 -> 2.5, C3 -> 3.75, C4 -> 5.0, ..., C8 -> 10.0
  const structuralSymScore =
    symTransforms > 0 ? Math.min(10, 2.5 + (symTransforms - 1) * 1.25) : 0
