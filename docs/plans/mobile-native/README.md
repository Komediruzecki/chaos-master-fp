...
- [x] Lifecycle: an `appActive` signal from `pause`/`resume` (`lib/lifecycle.ts`, over the `LifecyclePorts` seam), and `pause` writes the current flame as a draft (`lib/draft.ts`) that the next cold start offers back. Not ANDed into the `createAnimationFrame` accessors: both platforms suspend `requestAnimationFrame` for a hidden WebView and `finalRenderInterval` already parks the loop, so the accessor would be a second brake on a stopped wheel. Ancestry flushing and export checkpoints are still open.
