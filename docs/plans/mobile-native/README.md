...
- [x] Lifecycle: `pause` and `resume` over the `LifecyclePorts` seam (`lib/lifecycle.ts`), where `pause` writes the current flame as a draft (`lib/draft.ts`) that the next cold start offers back — native only, and only when something unsaved has actually changed. There is no `appActive` signal: it was deliberately not ANDed into the `createAnimationFrame` accessors (both platforms suspend `requestAnimationFrame` for a hidden WebView, and `finalRenderInterval` already parks the loop, so it would have been a second brake on a stopped wheel), which left it with no reader at all, so it has been removed rather than ticked. Ancestry flushing and export checkpoints are still open.
- [x] Android back button: closes the top modal, sheet, drawer, popover or destination, else `minimizeApp()`. The registry is `lib/backStack.ts`: every layer pushes its own handler while it is open, so the pop order is the order things opened and nothing inspects the DOM.
