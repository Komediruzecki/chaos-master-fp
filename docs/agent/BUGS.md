...
| 10 | [Taste records overwrite across sessions](#taste-profile-records-are-keyed-by-generation-candidateindex-only-so-every-new-director-session-overwrites-the-previous-sessions-ratings) | confirmed, medium | fixed, #90 (`404db62a`) | `packages/app/src/arcade/tasteStore.ts:184` (`feedback.sessionId`), `packages/app/src/components/DirectorOverlay.tsx:82` (`sessionId`); test `packages/app/src/arcade/tasteStore.test.ts:102` "keeps ratings from separate Director sessions that reuse generation numbers" |
...
| 27 | [Taste and stats skip transforms without `visible`](#extractflametastefeatures-and-calculategroundedstats-skip-every-transform-whose-visible-field-is-absent-which-is-the-normal-shape-of-an-agent-supplied-flame) | confirmed, low | open | `packages/app/src/arcade/tasteStore.ts:108` (`t.visible`), `packages/app/src/flame/stats.ts:518` (`t.visible`); `packages/app/src/webmcp/tools/arcadeDirector.ts:201` (`extractFlameTasteFeatures`) runs on the unparsed flame |
