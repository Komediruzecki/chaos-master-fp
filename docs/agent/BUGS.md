...
| 15 | [Variation-weight targets keyed by type](#variation-weight-targets-are-keyed-by-variation-type-so-two-same-type-variations-in-one-transform-collide-on-both-the-write-target-and-the-smoothing-state-key) | confirmed, low | open | `packages/app/src/utils/audioAnalysis.ts:847` (`tgt.variationType`), `:568` (`target.variationType`), `packages/app/src/utils/audioWiringPresets.ts:186` (`variation.type`) |
| 16 | [Variation-weight mappings turned live](#pr-82-silently-turned-every-variation-weight-audio-mapping-from-inert-to-live-inside-a-commit-labelled-a-refactor) | confirmed, low | open | `packages/app/src/utils/audioAnalysis.ts:850` (`weight = val`) writes the weight unbounded; the CHANGELOG still has no entry for the change |
