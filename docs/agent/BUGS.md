...
| 33 | [Core carries a TypeGPU shader function](#the-pure-core-package-contains-a-typegpu-shader-function-and-pulls-typegpu-into-the-cloudflare-workers-import-graph) | confirmed, low | open | `packages/core/src/math/affineTransform.ts:30` (`transformAffine`), exported from `packages/core/src/index.ts:3` (`affineTransform`), and reaching the worker through `packages/app/src/worker/routes/discord.ts:1` (`tryValidateFlame`); typegpu is still a runtime dependency of core, `packages/core/package.json:28` (`typegpu`). Since WP3b `pnpm arch` sees npm imports, and `core-declared-deps-only` lets typegpu through because core declares it: the rule holds core to its manifest, it does not settle this finding |
