...
| 27 | [Taste and stats skip transforms without `visible`](#extractflametastefeatures-and-calculategroundedstats-skip-every-transform-whose-visible-field-is-absent-which-is-the-normal-shape-of-an-agent-supplied-flame) | confirmed, low | open | `packages/app/src/arcade/tasteStore.ts:125` (`t.visible`), `packages/app/src/flame/stats.ts:526` (`t.visible`); `packages/app/src/webmcp/tools/arcadeDirector.ts:201` (`extractFlameTasteFeatures`) runs on the unparsed flame |
