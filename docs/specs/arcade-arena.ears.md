...
- `packages/app/src/components/WorkspaceModalsHost/WorkspaceModalsHost.tsx:92-100` — the mount gate for the overlay
...
- `packages/app/src/MainWorkspace.tsx:2786-2870` — the arena facade on the command context, `selectFighter`, and the placeholder `startClash`
- `packages/app/src/commands/types.ts:44-123` — `ArenaFighterStats` and the optional `arena` facade shape
...
- `packages/app/src/webmcp/tools/flameToolsModular.test.ts:127-158` — `open_arena` and `score_clash_round` execution contracts
- `packages/app/src/webmcp/webmcp.test.ts:352-540` — `open_arena`, `create_clash_flame` and `score_clash_round` through the bridge, including score determinism
...
- `packages/app/src/components/Arcade/ArcadeModePanel.test.tsx:74-104` — the Arena launch panel and its prompt card
...
_(`useWorkspaceArena.ts:63-77`; the same seeding runs whether entry came from the
Arcade hub's Launch Clash Arena button, the `open_arena` tool, or the effect at
`useWorkspaceArena.ts:121-125` that reacts to `showArena` flipping true.)_
...
_(`useWorkspaceArena.ts:79-110`)_
...
_(`useWorkspaceArena.ts:40-59` — the effect is `defer: true`, so mounting the workspace
...
_(`ArenaOverlay.tsx:1070`, `:1979`; the gallery picker re-provides its own `Root`
because the modal is portalled outside the app root, `:722-730`.)_
...
_(`ArenaOverlay.tsx:631-643` (`clearAllTimers`), `:645-654` (`captureWorkspace`),
`:656-678` (`restoreWorkspace`), `:853-856` (`onCleanup`), `:858-863` (`handleClose`).
...
_(`arenaArchetypes.ts:131-248` (the six recipes), `:278-333`)_
...
_(`arenaArchetypes.ts:281`, `:291-309`, via `mutateFlameSeeded`)_
...
_(`arenaArchetypes.ts:118-129` documents the rule; the type cannot enforce it, so
`arenaArchetypes.test.ts:81` pins the pools against the registries.)_
...
_(`ArenaOverlay.tsx:717` (`openGalleryForFighter`), `:766` (`handleSyncActiveFlame`),
`:795` (`handleApplySymmetry`), and the `disabled={gameState() === 'clashing'}` bindings
at `:1380`, `:1416`, `:1432`, `:1444`, `:1897`, `:1931`, `:1942`)_
...
_(`ArenaOverlay.tsx:716-763` (gallery; `CANCEL` and a null result both leave the slot
untouched), `:765-792` (sync))_
...
_(`ArenaOverlay.tsx:794-825`, pill wiring at `:1370-1380` and `:1887-1897`;
`applySymmetryToFlame` deletes every existing `_sym__` transform before adding
`folds - 1` new ones, `symmetry.ts:18-27`.)_
...
_(`stats.ts:96-110` (the coefficients), `:274-296`, `:332-498`)_
...
> **Known deviation:** `packages/app/src/flame/stats.ts:368` — the loop reads
...
_(`stats.ts:72-94`, `:204-248`; the fighter cards show `+25%` style advantage badges from
the same function, `ArenaOverlay.tsx:538-551`, rendered at `:1245` and `:1777`.)_
...
_(`createClashFlame.ts:111-137` (`calculatePowerSplit`), `:150-192`; `powerA`/`powerB`
...
_(`createClashFlame.ts:15-102`, `:194-314`, defaults at `:351-354` — note `tint`
...
_(`createClashFlame.ts:347-349`, `simulateClash.ts:194-196`)_
...
_(`scoreClashRound.ts:175-215`, `:385-406`; determinism is asserted at
`webmcp.test.ts:507-540`.)_
...
_(`scoreClashRound.ts:240-311`; the symmetry weight is `1 + min(0.3, symStrength * 0.05)`
...
_(`scoreClashRound.ts:348-361`, `:285-291`. The third branch is reached routinely:
...
_(`simulateClash.ts:126-154`, `:238-274`)_
...
_(`simulateClash.ts:156-163`, `:346-371`; `stats.ts:620-636` applies the override before
...
_(`stats.ts:503-648`; an unknown stance falls back to `balanced`, `:542-543`.)_
...
> **Known deviation:** `packages/app/src/webmcp/tools/simulateClash.ts:178-210` — the
...
_(`animateClash.ts:36-111`; the command path exists so the edit is recorded rather than
silently mutating state, `:92-100`.)_
...
_(`flameClashChoreography.ts:26-34` (the phase contract), `:39-62`, `:530-613`,
`:628-700`)_
...
_(`ArenaOverlay.tsx:925-973`, `:619-629` (`registerTimeout`), `:1073` (the class list);
the VFX callback re-checks `gameState() !== 'clashing'` before firing, `:945`.)_
...
_(`ArenaOverlay.tsx:1033-1071` (the handler and its input guard, removed on cleanup),
`:1011-1017` (`handleSkipClash`, which returns early if there is no cached simulation))_
...
_(`ArenaOverlay.tsx:975-1009`, log panel at `:1956-1978`)_
...
_(`ArenaOverlay.tsx:1022-1031`, `MainWorkspace.tsx:2851-2861`)_
...
> **Known deviation:** `packages/app/src/components/ArenaOverlay.tsx:104-170` —
...
_(`ArenaOverlay.tsx:566-568`. Note the button itself is not gated on a winner the way
Load Victor is (`:1677` vs `:1689-1700`), so on a draw the control is enabled and
...
_(`arenaGetStats.ts:30-56`; annotations at `simulateClash.ts:334-336`,
`scoreClashRound.ts:336-338`, `createClashFlame.ts:435-437`)_
...
_(`arenaCommentate.ts:27-47`; the overlay mirrors HUD state into its own local signals so
commentary survives a facade that implements only some setters, `ArenaOverlay.tsx:507-522`.)_
...
> **Known deviation:** `packages/app/src/webmcp/tools/arenaStartClash.ts:92-95` forwards
> `rounds: raw.rounds ?? 3` and `packages/app/src/commands/types.ts:118-121` carries it,
> but the only implementation reads `opts?.stance` alone
> (`packages/app/src/components/ArenaOverlay.tsx:835-844`) and `runSimulation` hard-codes
> `rounds: 3` (`ArenaOverlay.tsx:889`) and `framesPerRound: 30` (`:911`). A
...
`{ error: 'Arena clash startup timed out.' }` after 4 s — `MainWorkspace.tsx:2786-2810`.)_
...
- REQ-AA-024 (equal-probability and empty-transform branches) — only the missing-descriptor branch is covered (`flameToolsModular.test.ts:147`).
- REQ-AA-026 — `simulateClash.test.ts:138` exercises `Entangled` and `Collapse` only; `Nova`, `Symmetry Lock` and `Chaos Cascade` are unreached.
- REQ-AA-027 — nothing asserts that `territoryWinner` overrides the HP comparison.
- REQ-AA-028 (tie-break chain) — `stats.test.ts:149` checks one deterministic 3-round run; the HP → round-wins → beauty → draw ladder is untested.
...
- REQ-AA-032, REQ-AA-033 — spectator pacing, the VFX timers and the keyboard handler are untested; the `ArenaOverlay.test.tsx:165` results test drives `startClash` directly.
- REQ-AA-035 — neither `loadFighter` nor the workspace's `selectFighter` is exercised.
- REQ-AA-039 (`autoStart`) — `webmcp.test.ts:352` covers the stats/flame fallbacks but never sets `autoStart`.

**Covered in shape only** — a test names the code but would survive the requirement being
violated:

- REQ-AA-031 — `flameClashChoreography.test.ts:124,182` assert only that track paths are present; no test reads a single keyframe value off a `postAffine.d/c/h/l` track, so any of the twelve sign ternaries in `computeFighterXPosition` could be flipped and the suite would stay green. This is recorded as a test-gap finding in [docs/agent/BUGS.md](../agent/BUGS.md).
- REQ-AA-010 — `ArenaOverlay.test.tsx:147` asserts only that the rerolled opponent has a name and a flame; the state reset (rounds, battle log, cached simulation, restore) is not checked.
- REQ-AA-012 — `ArenaOverlay.test.tsx:135` asserts the buttons render, not what they do.
- REQ-AA-034 — `ArenaOverlay.test.tsx:165` asserts the trophy card renders; the winner mapping, the streak rules and the draw-leaves-streak branch are not asserted.
- REQ-AA-023 — `simulateClash.test.ts:86` checks ownership sums to 1 and `webmcp.test.ts:507` checks determinism; the `0.7/0.3` contested split, the symmetry weighting and the `0.01` verdict margin have no direct assertion.
